...
#include <include/contact_path_query.hpp>
#include <immintrin.h>
#include <chrono>
#include <iostream>
#include <sys/time.h>
...
bool test_end_to_end(ShuffleStore * shuffle_store,
  ...
  struct storable_path_encounter * bob_shuffleable = (struct storable_path_encounter *)
    aligned_alloc(32, (encounter_count + dummy_count) * sizeof(struct storable_path_encounter));
  ...
  free(bob_shuffleable);

  encounter::encounter * start_tile = &(original_encounters[encounter_count - (2 * tile_size)]);
  encounter::encounter * end_tile = &(original_encounters[encounter_count - tile_size]);
  emp::Integer output_count;
  emp::Integer source_device_ids[tile_size];
  emp::Integer sink_device_ids[tile_size];
  emp::Integer source_timestamps[tile_size];
  emp::Integer sink_timestamps[tile_size];
  ...
  contact<100, 3>( shuffle_store,
             revealer,
             &source_start_time_integer,
             &sink_end_time_integer,
             &max_duration_integer,
             source_device_ids, // encodes list of most recent known encounters
             source_timestamps,
             tile_size,
             sink_device_ids, // encodes list of most recent known encounters
             sink_timestamps,
             tile_size,
             &shuffle_id,
             &shuffle_start_time,
...
int runtests(int argc, char **argv) {
  int test, port, party;
  if (argc < 3) {
      std::cerr << "Usage: ./contact_path_query_test party port\n";
      std::exit(-1);
  }
  ...
  const std::string left_ip = "10.128.0.13"; // tdx01 is LEFT
  const std::string right_ip = "10.128.0.5"; // snp01 is RIGHT
  ...
  if(dualex::fork_and_setup(revealers,
    ...
    std::cout << "test results: " << test_end_to_end(shuffle_store,
                                                     ...
                                                     revealers,
                                                     shuffle_id,
                                                     1000, // encounter_count (MUST BE A MULTIPLE OF TILE SIZE)
                                                     1000,//617119904 - 1000, //dummy_count (MUST BE A MULTIPLE OF TILE SIZE)
                                                     dummy_nonce,
                                                     side,
                                                     10 // tile size
                                                     ) << std::endl << std::flush;
