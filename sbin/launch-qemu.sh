#!/bin/bash
# Edited from https://github.com/AMDESE/AMDSEV


# user parameters
kill $(pgrep -f qemu-system-x86)
sleep 5
...
# define guest memory
add_opts "-m ${MEM}M,slots=5,maxmem=103424M"
...
# do not use original network configuration, it is slow
...
#add_opts "-netdev tap,id=tap0,script=no,downscript=no"
#add_opts " -device virtio-net-pci,disable-legacy=on,iommu_platform=true,netdev=tap0,romfile="

# add port forwarding on the 1Gbps interface (for ssh connection, communication across VMs)
add_opts "-netdev user,id=vmnic,net=10.3.34.0/24,hostfwd=tcp::50022-:22,hostfwd=tcp::50000-:40000,hostfwd=tcp::50001-:40001"
add_opts "-device virtio-net-pci,netdev=vmnic,romfile="

# add the two 25Gbps interfaces for the direct connection to the mellanox machines
add_opts "-nic tap,model=virtio,ifname=tap0,script=no,downscript=no,vhost=on,vhostforce=on"
add_opts "-nic tap,model=virtio,ifname=tap1,script=no,downscript=no,vhost=on,vhostforce=on"
## alternative: old netdev option
## add_opts "-netdev tap,id=tap0,script=no,downscript=no -device virtio-net-pci,netdev=tap0,mac=52:54:00:12:34:56"
## add_opts "-netdev tap,id=tap1,script=no,downscript=no -device virtio-net-pci,netdev=tap1,mac=52:54:00:12:34:57"
